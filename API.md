...
公開順位はD1 read replicationを許可し、本人履歴・自己ベストはprimaryから読みます。応答の `x-d1-primary` は実際の読込先、`x-d1-region` はD1メタデータが返した地域です。削除後も別拠点のキャッシュで通常最大30秒、D1障害中のstale fallbackで最大5分、削除済みの公開行が表示される可能性があります。
