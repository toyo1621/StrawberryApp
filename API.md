...
Bearerなし・既定 `limit=30`・`no-cache` 指定なしの要求は実行拠点単位でキャッシュします。30秒以内は `hit`、初回は `miss`、再検証中またはD1障害時に5分以内の保存済み値を返す場合は `stale` です。それ以外は `bypass` です。投稿成功直後にクライアントが行うBearer付き要求はキャッシュを迂回し、`first-primary` で確定済みの順位を返します。投稿後は同じ拠点の対象モード・地域4期間を失効しますが、通常のBearerなし読込は別拠点で最大30秒古い結果を返す可能性があります。
