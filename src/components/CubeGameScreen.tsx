...
interface CubeGameScreenProps {
  onGameOver: (score: number) => void;
}

const CubeGameScreen: React.FC<CubeGameScreenProps> = ({ onGameOver }) => {
  ...
  const generateNewProblem = useCallback(() => {
    ...
    // 1から12までの数字をランダムに選択（立方数が大きくなりすぎないように）
    const number = Math.floor(Math.random() * 12) + 1;
    const correct = number * number * number;
    
    // 間違いの選択肢を生成（正解の±1〜50の範囲）
    let wrong;
    do {
      const offset = Math.floor(Math.random() * 100) + 1; // 1-100の差
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 w-full animate-pop-in">
      <div className="flex justify-between items-center mb-4 text-2xl font-bold">
        <div className="text-orange-500">スコア: {score}</div>
        <div className="text-gray-700">時間: {displayTime}</div>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-4 mb-6 overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-300 ease-linear ${
            timeLeft <= 100 ? 'bg-red-500' : 'bg-orange-400'
          }`}
        ...
        <div className="flex justify-around w-full max-w-sm">
          {answers.map((answer, index) => (
            <button
              ...
              className={`w-36 h-36 sm:w-40 sm:h-40 bg-orange-50 rounded-2xl flex items-center justify-center text-3xl font-bold transform transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-orange-300
