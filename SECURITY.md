...
- 許可Origin以外からのPOST/OPTIONSを拒否
- `application/json` と2 KiB本文上限を強制
- 名前の正規化、長さ・制御文字・危険なHTML文字の拒否
- モード、整数スコア、プレイ時間、成立可能な得点速度をWorkerで再検証
- 投稿IDを主キーにした冪等登録と競合検出
- IPアドレスとUser-Agentを秘密ソルト付きSHA-256にし、元データを保存しない連投制限
- 一時ハッシュを投稿時と15分ごとのcronで削除
