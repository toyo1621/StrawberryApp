...
本番はGitHub Pages、Cloudflare Worker、Cloudflare D1で構成します。互換性を保つため、リリース順序は **D1 Time Travel復元点、D1マイグレーション、read replication有効化、Worker、API v5とGit SHA確認、Pages** とします。Pages workflowが再利用可能なWorker workflowを呼ぶため、手動の公開順序には依存しません。
...
- `0011` は九州を北部・南部へ分割し、旧九州スコアは互換スコープに保全します。
- `0012` はCloudflare scheduled monitorの最新心拍を保持する1行テーブルを追加します。
- `0013` は本人マーカー照合をprimaryで高速化する所有者・スコープ別部分インデックスを追加します。
...
npx wrangler secret put MONITOR_ALERT_WEBHOOK_URL --config worker/wrangler.toml # 任意、HTTPSのみ
...
- Worker secret: `MONITOR_ALERT_WEBHOOK_URL`（任意。Cloudflare Cron障害・復旧通知）
- Environment secret: `MONITOR_ALERT_WEBHOOK_URL`（任意。GitHub外形監視の障害・復旧通知）
...
4. Git SHA付き `release.json` を含む本番Webビルドと660 KiB/raw JS・175 KiB/gzip JS・12 MiB/島SVG・14 MiB/全体の容量予算検査
5. Git SHAが一致するAPIを確認後にPages公開
6. 公開Webの `release.json`、APIヘルス、APIヘッダーが同じGit SHA・API v5であることをスモーク検査
...
公開前には [RELEASE_CHECKLIST.md](./RELEASE_CHECKLIST.md) に沿って実機で全4モード、振動、ダークモード、VoiceOver/TalkBack、文字拡大、オフライン復帰を確認し、各ストアのプライバシー回答と `store/privacy-declarations.json`、アプリ内ポリシーを一致させます。`extra.eas.projectId`、署名資格情報、審査結果は所有者アカウントで確認する外部証拠であり、設定前にストア公開済みとは扱いません。
