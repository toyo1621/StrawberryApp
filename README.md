...
- **🍓 いちごジュース**: 間違えた瞬間に画面全体にいちごジュースが表示され、3秒かけて透明になる（イライラ要素）
...
### 🚩 国旗モード
- **目標**: 指定された国の国旗を2つの選択肢から素早く選択
- **学習要素**: 世界の国旗を覚えながら楽しめる

### 🎨 色モード
- **目標**: 指定された色名を2つの選択肢から素早く選択
- **特別アイテム**: ✨ ゴールデン色（3点 + 1秒回復）
- **学習要素**: 色彩検定の色を学べる

## ゲームの遊び方

1. **モード選択**: いちごモード、島モード、国旗モード、色モードから選択
...
### 🏆 ランキングシステム
- **記録方法**: 各プレイヤーの最高スコアを永続保存（Supabase）
- **表示**: ゲーム終了後に30位まで表示
- **期間別ランキング**: 全体/日別/週別/月別でランキングを表示
- **個人スコア履歴**: マイページで自分のスコア履歴を確認可能
...
### ⚙️ 設定
- **ダークモード**: 画面を暗いテーマに変更
- **振動フィードバック**: 正解・不正解時のバイブレーションのオン/オフ

### 👤 マイページ
- **プレイヤー名の設定**: ランキングに表示される名前を設定
- **スコア履歴**: 自分の最高スコア履歴を確認
- **お問い合わせ**: 開発者へのお問い合わせフォーム
- **設定・その他**: 設定画面、プライバシーポリシー、利用規約へのアクセス
...
- **連続プレイ**: 途切れない快適なゲーム体験
- **ハプティックフィードバック**: 正解・不正解時の振動で直感的な操作感

### 🎨 ユーザビリティ
- **レスポンシブデザイン**: スマホ・PC両対応
- **アクセシビリティ**: 適切なaria-label設定
- **視覚的フィードバック**: アニメーション効果で直感的な操作感
- **中央配置レイアウト**: すべての画面でコンテンツを中央に配置
- **いちごジュース演出**: いちごモードで間違えた時に画面全体にいちごジュースが表示され、スケールアニメーションとフェードアウトで視覚的なフィードバックを提供

### 📊 データ分析対応
- **詳細なスコア記録**: プレイヤー別最高スコア管理
- **ゲームタイプ別集計**: モード別ランキング（いちご・島・国旗・色）
- **期間別ランキング**: 日別/週別/月別でのランキング表示
