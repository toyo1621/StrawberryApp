...
try {
  ...
  assert.equal(rankingsResult.body.some((entry) => entry.id === submissionId), true);
  ...
  assert.equal(deleteResult.body.deleted, 1);

  console.log('Local Worker/D1 integration passed: verified session, atomic write, retry, ranking, history, deletion.');
} finally {
  if (worker.exitCode === null) {
    worker.kill('SIGTERM');
    await Promise.race([
