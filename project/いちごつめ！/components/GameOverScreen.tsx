        ...
        <ul className="space-y-2">
          {ranking.length > 0 ? (
             ranking.slice(0, 5).map((entry, index) => (
              <li key={index} className={`flex justify-between items-center p-3 rounded-lg text-lg ${entry.name === currentPlayer.name && entry.score === currentPlayer.score ? 'bg-yellow-200 ring-2 ring-yellow-400' : 'bg-white'}`}>
