...
3. 終了時に名前、整数スコア、モード、経過時間、ランダムな投稿IDをBearerトークン付きでWorkerへ送ります。
...
| 保存先 | データ | 保持 |
| --- | --- | --- |
| D1 `rankings` | 投稿ID、公開名、スコア、モード、日時、所有者ハッシュ | 運営中または本人削除まで |
...
- 島の出題エリアは問題集合だけを変更し、API互換性を保つためランキングは既存の島モードで共通です。
