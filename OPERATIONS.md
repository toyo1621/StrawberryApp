...
- `.github/workflows/monitor-production.yml` が15分ごとに、Webシェル・実JS bundle・ファビコン・5秒以内の応答、全4モードと島8地域、CORS、API v4・release ID、D1接続、API各要求4秒以内を検査します。専用トークンでゲームセッションを発行し、テストスコアを登録、非公開履歴で確認し、1件以上削除して再取得時の不在まで検証します。
